Public Class Feedback
    ...
    Private Sub SendFeedback_DoWork(sender As Object, e As System.ComponentModel.DoWorkEventArgs) Handles SendFeedback.DoWork
        ...
        ElseIf TextBox3.Text = "" Then
            MsgBox("يلزم كتابة جسم الرسالة", MsgBoxStyle.Information)
            Exit Sub
            Button1.Enabled = True
        ElseIf TextBox4.Text = "" AndAlso CheckBox1.Checked = True Then
            MsgBox("يجب كتابة الموضوع (إقتراح - خطأ - ملاحظة - ...)", MsgBoxStyle.Information OrElse MsgBoxStyle.MsgBoxRight)
            Exit Sub
            Button1.Enabled = True
        End If
        Try
            ...
            e_mail.Dispose()
            MsgBox("تم الإرسال بنجاح", MsgBoxStyle.MsgBoxRight OrElse 0 OrElse MsgBoxStyle.Information)
        Catch ex As Exception
            If ex.Message = "The specified string is not in the form required for an e-mail address." Then
                MsgBox("الحساب غير موجود" & vbCrLf & "يرجى استخدام حساب صحيح.",, MsgBoxStyle.Information OrElse MsgBoxStyle.MsgBoxRight)
            ElseIf ex.Message = "Mailbox unavailable. The server response was: 5.7.3 Requested action aborted; user not authenticated" Then
                MsgBox("الحساب أو كلمة سر خاطئ " & vbCrLf & "يرجى استخدام حساب مايكروسوفت أو كتابة كلمة سر صحيحة", MsgBoxStyle.Information OrElse MsgBoxStyle.MsgBoxRight)
            ElseIf ex.Message = "Failure sending mail." Then
                MsgBox("حدث خطأ أثناء الإرسال." & vbCrLf & "يرجى التحقق من الاتصال بالانترنت.", MsgBoxStyle.MsgBoxRight OrElse MsgBoxStyle.Exclamation)
            ElseIf Paths.Instance.dev Then
                MsgBox(ex.Message)
            End If
        End Try
        Button1.Enabled = True
